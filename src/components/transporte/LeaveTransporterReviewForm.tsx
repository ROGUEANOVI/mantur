...
import { createTransporterReview } from '@/app/(app)/transporte/actions'
import { transportCopy } from '@/lib/copy/transport'

type FormState = { error: string } | { success: true } | undefined

async function createTransporterReviewFormAction(_prevState: FormState, formData: FormData): Promise<FormState> {
  return await createTransporterReview(formData)
}

// Calco de src/components/guias/LeaveReviewForm.tsx — solo calificación
// global + comentario, sin desglose por ítem (no aplica a un traslado).
export default function LeaveTransporterReviewForm({ transportRequestId }: { transportRequestId: string }) {
  const [open, setOpen] = useState(false)
  const [rating, setRating] = useState(0)
  const [hoverRating, setHoverRating] = useState(0)
  const [state, action, isPending] = useActionState<FormState, FormData>(createTransporterReviewFormAction, undefined)
  const copy = transportCopy.review
  ...
  return (
    <form action={action} className="w-full space-y-2 pt-2">
      <input type="hidden" name="transport_request_id" value={transportRequestId} />
