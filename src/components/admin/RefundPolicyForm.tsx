...
import { updateRefundPolicyRate } from '@/app/(app)/admin/reembolsos/actions'
import { adminCopy } from '@/lib/copy/admin'

type FormState = { error: string } | { success: true } | undefined

async function refundPolicyFormAction(
  _prevState: FormState,
  formData: FormData,
): Promise<FormState> {
  return (await updateRefundPolicyRate(formData)) ?? undefined
}
...
export default function RefundPolicyForm({ configId, minHoursBeforeBooking, currentRate }: Props) {
  const [state, action, isPending] = useActionState<FormState, FormData>(
    refundPolicyFormAction,
    undefined,
  )

  const label =
    adminCopy.reembolsos.policyTier[String(minHoursBeforeBooking)] ?? `≥ ${minHoursBeforeBooking}h`
  ...
  return (
    ...
    <form action={action} className="flex items-end gap-3">
      ...
      >
        {isPending
          ? adminCopy.reembolsos.saving
          : adminCopy.reembolsos.save}
      </button>
    </form>

    {isSuccess && (
      <p className="text-xs font-medium text-green-600 dark:text-green-400" role="status">
        {adminCopy.reembolsos.success}
      </p>
    )}
    {errorMsg && (
