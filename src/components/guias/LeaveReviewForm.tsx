...
import { createGuideTourReview } from '@/app/(app)/mis-reservas/actions'
import { bookingsCopy } from '@/lib/copy/bookings'

type FormState = { error: string } | { success: true } | undefined

async function createGuideTourReviewFormAction(_prevState: FormState, formData: FormData): Promise<FormState> {
  return await createGuideTourReview(formData)
}

export default function LeaveReviewForm({ bookingId }: { bookingId: string }) {
  const [open, setOpen] = useState(false)
  const [rating, setRating] = useState(0)
  const [hoverRating, setHoverRating] = useState(0)
  const [state, action, isPending] = useActionState<FormState, FormData>(createGuideTourReviewFormAction, undefined)
  const copy = bookingsCopy.review
